...
<?php
     ...
     $checkresult = mysqli_query($dbconnect, "SELECT * FROM `product` WHERE    `ProductName`='$productname';");
       if (mysqli_num_rows($checkresult) > 0) {                
      echo "Duplicate name, the reader already exists in the database.";
      }
	  else
	 {
     $sql="INSERT INTO reader (ReaderID, Name, Email, PhoneNo) values (NULL,'$name', '$email','$phoneno')"; //use NULL for auto number
