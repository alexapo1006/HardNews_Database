...
<?php
   ...
   $checkresult = mysqli_query($dbconnect, "SELECT * FROM `category` WHERE    `CategoryName`='$categoryname';");
       if (mysqli_num_rows($checkresult) > 0) {                
      echo "Duplicate name, the category already exists in the database.";
      }
	  else
	 {
     $sql="INSERT INTO category (CategoryID, CategoryName, ItemsCount, Area) values (NULL,'$categoryname', 0, '$area')"; //use NULL for auto number
